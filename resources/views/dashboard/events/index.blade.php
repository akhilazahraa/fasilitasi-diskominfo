                ...
                <ul class="dropdown-menu shadow-sm p-3 lh-lg">
                    @foreach ($providers as $providers)
                        <li><a class="dropdown-item "
                                href="/dashboard/events/filter/isp/{{ $providers->id }}">{{ $providers->name }}</a></li>
                    @endforeach
                </ul>
            </div>
                        ...
                        <tbody>
                            @foreach ($events as $item)
                                <tr class="lh-lg">
                                    ...
                                    <td>{{ $item->providers->name ?? 'Belum Ada' }}</td>

                                    <td>
                                        @if (auth()->user()->role == 'ADMIN')
                                            <a href="/dashboard/events/edit/{{ $item->id }}"
                                                class="button-action d-flex gap-2">
                                        ...
                                        @endif
                                        <a href="/dashboard/events/details/{{ $item->id }}"
                                            class="button-action d-flex gap-2">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
